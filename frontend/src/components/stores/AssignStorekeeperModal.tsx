...
export function AssignStorekeeperModal({
  ...
  const [selectedStorekeeper, setSelectedStorekeeper] = useState<number | null>(null);
  const [assignmentType, setAssignmentType] = useState<'warehouse' | 'store'>('warehouse');
  const [selectedStores, setSelectedStores] = useState<string[]>([]);

  const currentStorekeeper = storekeepers.find((sk) => sk.id === selectedStorekeeper);
  ...
  // Update form when storekeeper is selected
  useEffect(() => {
    if (currentStorekeeper) {
      setAssignmentType(currentStorekeeper.assignment_type);
      setSelectedStores(currentStorekeeper.assigned_store_ids.map(String));
    }
  }, [currentStorekeeper]);
    ...
    >
      <Stack gap="md">
        <Alert icon={<IconInfoCircle size={16} />} color="blue" variant="light">
          Assign storekeepers to specific stores or give them access to all stores in the
          warehouse.
        </Alert>
            ...
            {storekeepers.map((storekeeper) => (
              <Group
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  backgroundColor:
                    selectedStorekeeper === storekeeper.id ? '#f1f3f5' : 'transparent',
                }}
                onClick={() => setSelectedStorekeeper(storekeeper.id)}
              >
                <Radio
                  checked={selectedStorekeeper === storekeeper.id}
                  onChange={() => setSelectedStorekeeper(storekeeper.id)}
                />
                <div style={{ flex: 1 }}>
                  <Group gap="xs">
                    <IconUser size={16} />
                    ...
                    <Badge
                      size="sm"
                      color={storekeeper.assignment_type === 'warehouse' ? 'blue' : 'green'}
                    >
                  ...
                  {storekeeper.assignment_type === 'store' &&
                    storekeeper.assigned_stores.length > 0 && (
                      <Text size="xs" c="dimmed" mt={4}>
                        ...
                        {storekeeper.assigned_stores.map((s) => s.name).join(', ')}
                      </Text>
                    )}
                </div>
              </Group>
        ...
        {selectedStorekeeper && (
          <>
            ...
            <div>
              ...
              <Radio.Group value={assignmentType} onChange={(value) => setAssignmentType(value as 'warehouse' | 'store')}>
                <Stack gap="xs">
                  <Radio
                    value="warehouse"
            ...
            <Alert color="gray" variant="light">
              ...
              <Text size="sm">
                ...
                {assignmentType === 'warehouse' ? (
                  <strong>all stores in {currentStorekeeper?.warehouse_name}</strong>
                ) : (
                  <strong>
                    {selectedStores.length} selected store(s)
                  </strong>
                )}
              </Text>
            </Alert>
