...
RSpec.describe "POST /cats_warehouse/v1/receipt_authorizations/:id/assign_storekeeper", type: :request do
  def auth_headers_for(user)
    { "Authorization" => "Bearer #{user.signed_id(purpose: "auth", expires_in: 1.hour)}" }
  end
  ...
  let(:sk_other) { create(:cats_core_user, role_name: "Storekeeper") }
  let(:hm) { create(:cats_core_user, role_name: "Hub Manager") }
  let(:commodity) { create(:cats_core_commodity) }
  let(:unit) { commodity.unit_of_measure }
  let(:transporter) { create(:cats_core_transporter) }
  ...
  before do
    ...
    Cats::Warehouse::UserAssignment.create!(user: sk_other, warehouse: warehouse, role_name: "Storekeeper")
    allow(Cats::Warehouse::NotificationFanout).to receive(:deliver)
  end

  it "assigns a storekeeper and exposes assignment fields" do
    post "/cats_warehouse/v1/receipt_authorizations/#{ra.id}/assign_storekeeper",
         params: { payload: { storekeeper_user_id: sk.id } },
         headers: auth_headers_for(wm),
         as: :json

    expect(response).to have_http_status(:ok)
    body = JSON.parse(response.body)
    expect(body.dig("data", "assigned_storekeeper_id")).to eq(sk.id)
    expect(body.dig("data", "awaiting_storekeeper_assignment")).to eq(false)

    get "/cats_warehouse/v1/receipt_authorizations",
        headers: auth_headers_for(sk),
        as: :json

    ids = JSON.parse(response.body).fetch("data").map { |row| row["id"] }
    expect(ids).to include(ra.id)

    get "/cats_warehouse/v1/receipt_authorizations",
        headers: auth_headers_for(sk_other),
        as: :json

    other_ids = JSON.parse(response.body).fetch("data").map { |row| row["id"] }
    expect(other_ids).not_to include(ra.id)
  end

  it "forbids warehouse manager at another warehouse" do
    post "/cats_warehouse/v1/receipt_authorizations/#{ra.id}/assign_storekeeper",
         params: { payload: { storekeeper_user_id: sk.id } },
         headers: auth_headers_for(other_wm),
         as: :json

    expect(response).to have_http_status(:forbidden)
  end

  it "allows reassign while pending without inspections" do
    post "/cats_warehouse/v1/receipt_authorizations/#{ra.id}/assign_storekeeper",
         params: { payload: { storekeeper_user_id: sk.id } },
         headers: auth_headers_for(wm),
         as: :json
    ...
    post "/cats_warehouse/v1/receipt_authorizations/#{ra.id}/assign_storekeeper",
         params: { payload: { storekeeper_user_id: sk_other.id } },
         headers: auth_headers_for(wm),
         as: :json

    expect(response).to have_http_status(:ok)
    body = JSON.parse(response.body)
    expect(body.dig("data", "assigned_storekeeper_id")).to eq(sk_other.id)
  end
end
